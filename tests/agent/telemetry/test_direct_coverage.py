...
def test_endpoint_validator_direct():
    ...
    source_lines = inspect.getsourcelines(mock_otel.DaprAgentsOTel._endpoint_validator)[0]
    ...
    print(f"URL.endswith('/v1/{signal_type}'): {url_with_slash.endswith(f'/v1/{signal_type}')}")
    ...
    print(f"URL.endswith('/v1/{signal_type}'): {url_with_path.endswith(f'/v1/{signal_type}')}")
