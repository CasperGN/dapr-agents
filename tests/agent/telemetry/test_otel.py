...
# Import our mock classes first
from tests.agent.telemetry.mock_otel import (
    Resource as MockResource,
    TracerProvider as MockTracerProvider,
    MeterProvider as MockMeterProvider,
    LoggerProvider as MockLoggerProvider,
    Context as MockContext,
)

# Try importing necessary OpenTelemetry modules
try:
    ...
    try:
        ...
        try:
            from opentelemetry.sdk.trace.sampling import (
                ParentBased as ParentBasedSampler,
            )
        except ImportError:
    ...
    trace = MockClass()
    context = MockClass()
    # Use our mock classes from mock_otel.py instead
    Resource = MockResource
    TracerProvider = MockTracerProvider
    MeterProvider = MockMeterProvider
    LoggerProvider = MockLoggerProvider
    Context = MockContext
    ParentBasedSampler = MockClass
...
def otel_instance():
    ...
    return DaprAgentsOTel(
        service_name="test-service", otlp_endpoint="http://localhost:4318"
    )
...
class TestDaprAgentsOTel:
    ...
    def test_init_with_defaults(self, monkeypatch):
        ...
        monkeypatch.setattr(
            Resource, "create", mock.MagicMock(return_value=mock.MagicMock())
        )
    ...
    def test_init_with_params(self, monkeypatch):
        ...
        otel = DaprAgentsOTel(
            service_name="custom-service", otlp_endpoint="http://custom-endpoint:4318"
        )
    ...
    def test_setup_resources(self, otel_instance):
        ...
        # Check if it's either a real Resource or our mock Resource
    ...
    def setup_mocks(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel.TracerProvider",
            lambda *args, **kwargs: self.mock_tracer_provider,
        )
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel.MeterProvider",
            lambda *args, **kwargs: self.mock_meter_provider,
        )
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel.LoggerProvider",
            lambda *args, **kwargs: self.mock_logger_provider,
        )

        # Patch the set_provider functions
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel.set_tracer_provider", self.mock_set_tracer
        )
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel.set_meter_provider", self.mock_set_meter
        )
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel.set_logger_provider", self.mock_set_logger
        )
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel.LoggingHandler",
            lambda *args, **kwargs: self.mock_logging_handler,
        )
    ...
    def test_create_logger_provider(
        self, otel_instance, mock_logger, mock_env_vars, setup_mocks, monkeypatch
    ):
    ...
    def test_endpoint_validator_no_trailing_slash(self, otel_instance, monkeypatch):
        ...
        assert execution_record["line_129_executed"]
...
class TestMockProviders:
    ...
    def test_provider_methods(self):
        ...
        from tests.agent.telemetry.mock_otel import LoggingHandler
        import logging
        ...
        meter_provider = MockMeterProvider(metric_readers=[reader])
        ...
        record = logging.LogRecord(
            "test", logging.INFO, __file__, 1, "Test message", None, None
        )
        handler.emit(record)

        # Use provider classes imported at module level for the other tests
        # flake8: noqa: E402, F841
        from tests.agent.telemetry.mock_otel import TracerProvider as TP
        from tests.agent.telemetry.mock_otel import MeterProvider as MP
        from tests.agent.telemetry.mock_otel import LoggerProvider as LP

        # Test with different variable names to avoid redefinition
        provider1 = TP()
        provider1.add_span_processor(mock.MagicMock())

        reader = mock.MagicMock()
        meter_provider = MP(metric_readers=[reader])
        assert reader in meter_provider.metric_readers

        logger_provider = LP()
        processor = mock.MagicMock()
        logger_provider.add_log_record_processor(processor)
        _ = logger_provider.get_logger("test_logger", "1.0")
...
class TestContextPropagation:
    ...
    def setup_mocks(self, monkeypatch):
        ...
        # Apply patches to our mock module
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel._propagator.inject",
            self.mock_propagator_inject,
        )
        monkeypatch.setattr(
            "tests.agent.telemetry.mock_otel._propagator.extract",
            self.mock_propagator_extract,
        )
