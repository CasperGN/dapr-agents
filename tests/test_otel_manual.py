...
# This comment is used to satisfy flake8 that we need sys.path modification before import
# flake8: noqa: E402
...
def test_init_with_defaults():
    ...
    otel = DaprAgentsOTel()
    assert (
        otel.service_name == "dapr-agents"
    ), f"Expected service_name to be 'dapr-agents', got {otel.service_name}"
    assert (
        otel.deployment_environment == "development"
    ), f"Expected deployment_environment to be 'development', got {otel.deployment_environment}"
...
def test_init_with_params():
    ...
    assert (
        otel.service_name == "custom-service"
    ), f"Expected service_name to be 'custom-service', got {otel.service_name}"
    assert (
        otel.otlp_endpoint == "http://custom-endpoint:4318"
    ), f"Expected otlp_endpoint to be 'http://custom-endpoint:4318', got {otel.otlp_endpoint}"
...
def test_init_with_env_vars():
    ...
    try:
        ...
        ), f"Expected service_name to be 'test-service', got {otel.service_name}"
        assert (
            otel.deployment_environment == "testing"
        ), f"Expected deployment_environment to be 'testing', got {otel.deployment_environment}"
    finally:
...
def test_setup_resources():
    ...
    assert (
        attributes.get("service.name") == "test-service"
    ), f"Expected service.name to be 'test-service', got {attributes.get('service.name')}"
    assert (
        "service.instance.id" in attributes
    ), "service.instance.id not found in attributes"
...
def test_endpoint_validator_http():
    ...
    assert (
        result == "http://localhost:4318/v1/traces"
    ), f"Expected 'http://localhost:4318/v1/traces', got {result}"

    # Test with URL missing path
    url_no_path = "http://localhost:4318"
    result = otel._endpoint_validator(url_no_path, "metrics")
    assert (
        result == "http://localhost:4318/v1/metrics"
    ), f"Expected 'http://localhost:4318/v1/metrics', got {result}"
...
def test_endpoint_validator_https():
    ...
    assert (
        result == "https://otel-collector.example.com:4318/v1/logs"
    ), f"Expected 'https://otel-collector.example.com:4318/v1/logs', got {result}"
